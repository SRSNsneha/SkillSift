...
@feature1_routes.route("/store_resume", methods=["POST"])
def store_resume():
...
@feature1_routes.route("/match_resumes", methods=["POST"])
def match_resumes():
...
@feature1_routes.route("/download_resume/<name_id>/<filename>")
...
@feature1_routes.route("/clear_resumes", methods=["DELETE"])
def clear_resumes():
    ...
    return jsonify({"message": f"{len(deleted)-1} resume(s) deleted successfully."})



@feature1_routes.route("/check_user_id")
def check_user_id():
...
@feature1_routes.route("/store_user_pin", methods=["POST"])
def store_user_pin():
    data = request.get_json()
    name_id = data.get("name_id", "").strip().lower()
...
@feature1_routes.route("/verify_pin", methods=["GET"])
def verify_pin():
    name_id = request.args.get("name_id", "").strip().lower()
    pin = request.args.get("pin", "").strip()
